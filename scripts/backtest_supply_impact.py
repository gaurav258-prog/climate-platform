...
def main():
    ...
    print("-" * 78)
    print(f"  • Direction: 2/2 correct — supply down → price up.")
    print(f"  • Magnitude: a CONSTANT-elasticity model is wrong, and wrong in BOTH directions:")
    c = rows[0]; cof = rows[1]
    ...
    print(f"  • ⇒ The stock-to-use AMPLIFICATION term (methodology §1.3) is ESSENTIAL, and it is steep:")
    print(f"        implied A ≈ {c[2]:.0f}–{c[3]:.0f}× at {c[0]['stock_to_use']}% stocks  vs  A ≈ {cof[2]:.1f}× at {cof[0]['stock_to_use']}% stocks.")
    print(f"        BUT two points is a DIRECTION, not a calibrated curve — a full stocks-to-use panel is needed.")
    print(f"  • Attribution: even amplified, cocoa's +{c[0]['realised'][1]}% peak is not 100% climate — EUDR/disease/")
    print(f"        speculation contributed; we report the climate-attributable SHARE with a band, never the whole move.")
    print(f"  • {WHEAT_NOTE}")
