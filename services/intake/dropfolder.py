...
matching, 4-eyes. The channel declares the files' currency; each file carries a book_date column (the date its
figures describe) or is refused with that reason. The channel's owner stands as the sender, so a batch with a failed check goes to a DIFFERENT person.
...
def create_channel(session: Session, org_id: str, template: str, owner_user_id: str, created_by: str,
                   mapping_profile_id: Optional[str] = None, currency: Optional[str] = None) -> dict:
    ...
    try:
        ccy, _ = validate_declaration(session, currency, None)
    except MoneyError as e:
        raise ChannelError(str(e))
    if not ccy:
        raise ChannelError("Declare the currency of this folder's files (a file's own currency column overrides it).")
    folder = f"{org_id}/{template}"
    row = session.execute(text("""
        INSERT INTO intake_channels (org_id, template, folder, owner_user_id, mapping_profile_id, created_by, currency)
        VALUES (CAST(:o AS uuid), :t, :f, CAST(:u AS uuid), CAST(:m AS uuid), CAST(:c AS uuid), :ccy)
        ON CONFLICT (org_id, template, kind) DO NOTHING RETURNING channel_id::text
    """), {"o": org_id, "t": template, "f": folder, "u": owner_user_id, "m": mapping_profile_id, "c": created_by, "ccy": ccy}).scalar()
    if not row:
...
def get_channel(session: Session, org_id: str, channel_id: str) -> Optional[dict]:
    r = session.execute(text("""
        SELECT c.channel_id::text, c.template, c.folder, c.owner_user_id::text, u.email AS owner_email,
               c.mapping_profile_id::text, c.enabled, c.created_at, c.last_swept_at, c.currency
    ...
    with get_session() as s:
        try:
            out = submit(s, org_id, channel["template"], raw, name, via="sftp", user_id=channel["owner_user_id"],
                         ...
                         currency=channel.get("currency"))   # book date: each file's own book_date column
            s.commit()
...
def sweep_all() -> dict:
    ...
    with get_session() as s:
        chans = s.execute(text("""SELECT channel_id::text, org_id::text, template, folder, owner_user_id::text,
                                         mapping_profile_id::text, currency FROM intake_channels WHERE enabled""")).mappings().all()
