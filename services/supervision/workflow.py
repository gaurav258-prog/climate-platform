...
def _seen(session, regulator_org_id: str, subject_org_id: str, action: str) -> Optional[str]:
    return session.execute(text("""
        SELECT max(created_at) FROM access_audit_log
        WHERE org_id = CAST(:s AS uuid) AND action = :a AND (detail->>'regulator_org_id') = :r
    """), {"s": subject_org_id, "r": regulator_org_id, "a": action}).scalar()


def _lens_seen(session, regulator_org_id: str, subject_org_id: str) -> Optional[str]:
    return _seen(session, regulator_org_id, subject_org_id, "supervisor.lens.access")


def _tier1_seen(session, regulator_org_id: str, subject_org_id: str) -> Optional[str]:
    return _seen(session, regulator_org_id, subject_org_id, "supervisor.plausibility.access")
...
def entity_workflow(session, regulator_org_id: str, e: dict, in_profile: bool, submission: Optional[dict],
    ...
    tier1 = _tier1_seen(session, regulator_org_id, e["org_id"]) if has_sub else None
    reviewed = seen is not None or (not has_granular and tier1 is not None)
    steps = [
        ...
        {"key": "reviewed", "label": STEP_LABEL["reviewed"], "done": reviewed, "partial": (tier1 is not None and has_granular and seen is None),
         "detail": (f"lens run {seen.date().isoformat()} · {n_flagged} questions" if seen is not None else
                    f"Tier 1 plausibility checked {tier1.date().isoformat()} (template only)" if (tier1 is not None and not has_granular) else
                    "lens ready to run" if has_sub and has_granular else
                    "template on file — Tier 1 plausibility ready" if has_sub else "needs step 2 first")},
    ...
    elif has_sub and not has_granular and tier1 is None:
        stage, nxt = "submitted", {"label": "Check the submitted template's plausibility (Tier 1)", "to": f"/supervised/{e['org_id']}/plausibility"}
    elif has_sub and not has_granular:
        stage, nxt = ("engaged" if eng["n"] else "reviewed"), (
            {"label": f"Follow up: {eng['n_open']} open", "to": f"/supervisor/requests?entity={e['org_id']}"} if eng["n_open"] else
            {"label": "All requests closed — monitor", "to": f"/supervisor/requests?entity={e['org_id']}"} if eng["n"] else
            {"label": "Raise anything outside the band with the entity, or ingest granular data for the full lens", "to": f"/supervised/{e['org_id']}/plausibility"})
