...
export default function RiskMapBank() {
  ...
  // Volcanic/storm/pollution exposure are each a small pocket (Guatemala, Puerto
  // Rico, Delhi/California) in an otherwise-European book — centering on the whole
  // book's average would bury them. Let RiskMap's own per-hazard default view take
  // over instead of the asset average. (Same bug class already caught once for
  // storm — extending the same fix here up front instead of rediscovering it later.)
  const view = (hazard === 'volcanic' || hazard === 'storm' || hazard === 'pollution') ? null : assetView
