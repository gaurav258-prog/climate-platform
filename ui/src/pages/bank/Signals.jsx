...
import { Radio, Activity, MapPin, ShieldCheck } from 'lucide-react'
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8">
        ...
        <div className="grid grid-cols-3 gap-5">
            ...
            {nearby.length ? (
              <ul className="mt-3 divide-y divide-gray-100">
                {nearby.map(({ e, km, asset }) => (
                  <li key={e.event_id} className="flex items-center justify-between py-2.5">
                    <div>
          ...
          <section className="rounded-2xl border border-gray-200/70 bg-white p-5 shadow-sm">
            <h2 className="flex items-center gap-2 text-[14px] font-semibold text-[#1d1d1f]">
              <Activity size={15} className="text-[#0071e3]" /> Model check
            </h2>
            <p className="mt-0.5 text-[11px] text-gray-500">Forecast vs reality, daily</p>
            {verif ? (
              <div className="mt-3">
                <div className="text-[12px] text-gray-500">Venezuela M7.5 aftershocks</div>
        ...
        <section className="mt-5 rounded-2xl border border-gray-200/70 bg-white p-5 shadow-sm">
          ...
          <ul className="mt-3 grid grid-cols-2 gap-x-8 gap-y-1">
            {events.slice(0, 16).map(e => (
              <li key={e.event_id} className="flex items-center gap-2 border-b border-gray-50 py-1.5 text-[12px]">
