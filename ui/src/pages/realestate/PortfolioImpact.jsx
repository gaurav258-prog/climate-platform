...
import { Building2, TrendingDown, Layers, ShieldAlert, Download, FileSpreadsheet, Leaf, ChevronDown } from 'lucide-react'
                  ...
                  <table className="mt-3 w-full text-[12px]">
                    ...
                    <tbody>
                      {Object.entries(disclosure.by_hazard).map(([h, v]) => {
                        ...
                        const top = open
                          ? properties.filter(p => p.hazards?.some(hz => hz.hazard === h && (hz.bucket === 'H' || hz.bucket === 'VH')))
                              ...
                              <tr><td colSpan={3} className="bg-[#f5f5f7] px-2 py-1.5">
                                {top.length ? (
                                  <div className="divide-y divide-gray-200/70">
                                    {top.map(p => (
                                      <button key={p.property_id} onClick={e => { e.stopPropagation(); setSel(p.property_id) }}
                                        className="flex w-full items-center justify-between py-1.5 text-left hover:opacity-70">
                                        <span className="text-[11px] text-[#1d1d1f]">{p.property_name}</span>
                                        <span className="text-[11px] font-medium text-gray-500">
                                          {p.hazards.find(hz => hz.hazard === h)?.score?.toFixed(1)}
                                        </span>
                                      </button>
                                    ))}
                                  </div>
                                ) : <p className="py-1.5 text-[11px] text-gray-400">No High+ properties for this hazard.</p>}
                              </td></tr>
                            )}
                          </Fragment>
                        )
