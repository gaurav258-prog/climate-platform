...
function OverallCard({ baseline, current, heatStatus }) {
  ...
  const b = BUCKET[baseline.bucket] || BUCKET.L
  const worseNow = current.score != null && baseline.score != null && current.score > baseline.score
  return (
    <div className="rounded-2xl border border-gray-200/70 p-6 shadow-sm" style={{ background: b.bg }}>
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[12px] font-medium uppercase tracking-wide" style={{ color: b.c }}>Baseline risk score</p>
          <p className="mt-1 text-5xl font-semibold tabular-nums" style={{ color: b.c }}>{baseline.score}</p>
          <p className="mt-1 text-[12px] text-gray-500">What this place is like on a typical day</p>
        </div>
        <div className="text-right">
          <span className="inline-block rounded-lg bg-white px-3 py-1 text-[13px] font-semibold" style={{ color: b.c }}>
            {baseline.bucket} · {b.label}
          </span>
          {baseline.driver_hazard && (
            <p className="mt-2 text-[12px]" style={{ color: b.c }}>
              driven by {HAZ_LABEL[baseline.driver_hazard] || baseline.driver_hazard}
            </p>
          )}
        </div>
      </div>
      ...
      {heatStatus?.elevated && (
        <div className="mt-4 flex items-start gap-2 rounded-xl border border-orange-200 bg-orange-50 px-3 py-2.5 text-[12.5px] text-orange-700">
          <ThermometerSun size={15} className="mt-0.5 shrink-0" />
          <span>
            Heat is elevated right now — {heatStatus.acute_score} today vs a typical {heatStatus.baseline_score}.
            This doesn't change the baseline above, but it's reflected in the worst-case figure below.
          </span>
        </div>
      )}

      {worseNow && (
        <div className="mt-3 flex items-center justify-between rounded-lg bg-white/60 px-3 py-2 text-[12.5px]">
          <span className="text-gray-500">Worst case right now — use for due-diligence / disclosure</span>
          <span className="font-semibold tabular-nums" style={{ color: (BUCKET[current.bucket] || BUCKET.L).c }}>
            {current.score} · {current.bucket}
          </span>
        </div>
      )}
      ...
      {baseline.status === 'provisional' && (
        <p className="mt-4 flex items-center gap-1.5 text-[12px]" style={{ color: b.c }}>
          ...
          provisional — {baseline.hazards_pending} more hazard{baseline.hazards_pending === 1 ? '' : 's'} still computing
        </p>
      )}
    </div>
  )
...
export default function LookupScorePage({ onHome }) {
  ...
  const pollOne = useCallback((lookupId) => {
    ...
    const tick = async () => {
      ...
      try {
        const { hazard, overall, baseline, heat_status } = await pollLookup(lookupId)
        setResult(prev => {
          if (!prev) return prev
          const hazards = prev.hazards.map(h => (h.lookup_id === lookupId ? { ...h, ...hazard, hazard_type: h.hazard_type } : h))
          return { ...prev, hazards, overall, baseline, heat_status }
        })
      ...
      <div className="mx-auto max-w-xl px-6 pb-20 pt-6">
        ...
        {result && (
          <div className="mt-8">
            ...
            <div className="mt-3">
              <OverallCard baseline={result.baseline} current={result.overall} heatStatus={result.heat_status} />
            </div>
