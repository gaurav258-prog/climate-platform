...
interface RiskScore {
  ...
  aftershock_24h: number;
  aftershock_72h: number;
  aftershock_7d: number;
  computed_at: string;
}

interface SeismicRiskMapProps {
  riskScores?: RiskScore[];
  onCellClick?: (cell: RiskScore) => void;
}

export const SeismicRiskMap: React.FC<SeismicRiskMapProps> = ({
  riskScores = [],
  onCellClick,
}) => {
  const [scores, setScores] = useState<RiskScore[]>(riskScores);
...
const RiskHexagon: React.FC<RiskHexagonProps> = ({ score, color, label, onClick }) => {
  const CircleMarker = ({ center, radius, color }: any) => {
