...
interface ForecastWindow {
  ...
  end_time: string;
  probability: number;
  expected_magnitude_range: [number, number];
  expected_aftershock_count: number;
...
export const AftershockForecastCard: React.FC<AftershockForecastCardProps> = ({
  eventId,
  onClose,
}) => {
  const [forecast, setForecast] = useState<AftershockForecast | null>(null);
  ...
  const getProbabilityBg = (prob: number): string => {
