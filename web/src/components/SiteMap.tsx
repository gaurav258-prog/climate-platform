...
export default function SiteMap({ points, selectedId, onSelect, color }:
  { points: SitePoint[]; selectedId?: string | null; onSelect?: (id: string) => void; color: (score: number) => [number, number, number] }) {
  const dark = useDark()
  ...
  return (
    <div className="relative w-full h-full min-h-[360px] rounded-xl overflow-hidden border border-[var(--color-line)]">
      <MapContainer center={center} zoom={4} minZoom={2} maxZoom={16} scrollWheelZoom={false} worldCopyJump
        ...
        {located.map(p => {
          const [r, g, b] = color(p.score ?? 0)
          const sel = p.id === selectedId
          return (
            <CircleMarker key={p.id} center={[p.lat, p.lon]} radius={sel ? 9 : 6}
              pathOptions={{ color: sel ? (dark ? '#fff' : '#0f172a') : `rgb(${r},${g},${b})`, weight: sel ? 2.5 : 1,
                             fillColor: `rgb(${r},${g},${b})`, fillOpacity: p.score == null ? 0.35 : 0.85 }}
      ...
      <div className="absolute left-2 bottom-2 z-[500] mono text-[10px] px-2 py-1 rounded-md bg-[var(--color-panel)]/90 text-[var(--color-faint)] border border-[var(--color-line)]">
        {located.length} of {points.length} sites located · click a dot to open it
      </div>
    </div>
  )
}
